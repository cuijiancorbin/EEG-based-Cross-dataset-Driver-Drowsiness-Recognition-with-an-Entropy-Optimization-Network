...
"""
 The code implements Entropy Optimization Network (EON) for cross-dataset driver drowsiness recognition:
...
#  change this between 0 and 1 to switch the direction of transfer 
    ...
    print('mean accuracy after Step 1:',np.mean(results))
    ...
    for epoch in range(stage_two_epoch):
           ...
           with torch.no_grad():
               for i in range(1, subjnum + 1):
                   ...
                   pseudo_label_list.extend(preds)
                   probs_list.extend(probs)
                   #
                   # acc = accuracy_score(y_test, preds)
                   # results[i - 1] = acc
               p = 0.8+0.2*epoch/stage_two_epoch
               selected_data,selected_label = construct_pseudo_dataset(probs_list,pseudo_label_list,testdatax,testdatay,p)
               print('mean accuracy after {} epoch self-training:{}'.format(epoch,np.mean(results)))
           ...
           for j, data in enumerate(train_loader, 0):
               ...
               slctidx = np.random.choice(len(selected_data), labels.size()[0], replace=False)
               xtestbatch = selected_data[slctidx]
               xtestbatch = xtestbatch.reshape(xtestbatch.shape[0], 1, channelnum, samplelength * sf)
               xtestbatch = torch.DoubleTensor(xtestbatch).cuda()
               ytestbatch = torch.LongTensor(selected_label[slctidx]).cuda()
               ...
               err_s_label = loss_class(class_output, class_label)
               err_t_label = loss_class(targetout,ytestbatch)
...
def construct_pseudo_dataset(probs,preds,data,label,p):
    entropy = -np.sum(np.exp(probs) * (probs),axis=-1)

    copied_entropy = deepcopy(entropy)
    copied_entropy.sort()
    treshold = copied_entropy[int(len(copied_entropy)*p)]

    mask = np.squeeze(entropy<treshold)
    preds = np.squeeze(preds)

    selected_data = data[mask]
    selected_label = preds[mask]
